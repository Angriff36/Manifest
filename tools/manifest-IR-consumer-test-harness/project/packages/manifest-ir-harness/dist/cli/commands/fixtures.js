...
export const fixturesCommand = new Command('fixtures')
    ...
    for (const fixture of fixtures) {
        ...
        try {
            ...
            console.log(`  ${icon} ${fixture.name} — ${result.summary.assertionsPassed}/${result.summary.assertionsPassed + result.summary.assertionsFailed} assertions`);
            if (result.summary.failed > 0) {
                totalFailed++;
                for (const step of result.execution.steps) {
                    for (const detail of step.assertions.details) {
                        if (!detail.passed) {
                            console.log(`         step ${step.step}: ${detail.check} — expected ${JSON.stringify(detail.expected)}, got ${JSON.stringify(detail.actual)}`);
                        }
                    }
                }
            }
