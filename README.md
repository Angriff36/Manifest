...
- **Projections System**: Generate platform-specific code from IR.
  ...
  - **Prisma projection** (v0.9.0+; folded into the main package as of v0.9.2) emits a `schema.prisma` from compiled IR with full `@relation` wiring, decimal precision for `money`/`decimal` types, and a hard `PRISMA_AMBIGUOUS_NUMBER` diagnostic for bare `number` columns (loud at compile time beats silent rounding in production). App-agnostic by construction. Import: `@angriff36/manifest/projections/prisma`.
