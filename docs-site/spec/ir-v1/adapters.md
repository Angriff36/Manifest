---
title: "Adapters"
description: "This document defines adapter hooks for storage targets and action kinds. Adapters are extensions, not core language features, unless stated otherwise."
---

<!-- GENERATED FROM docs/spec/adapters.md @ b205b62 — edit the source under /docs and run `node scripts/docs-sync.mjs`. -->
...
See [guides/implementing-custom-stores.md](/patterns/implementing-custom-stores) for complete examples:
...
For transactional outbox patterns, stores MAY support event collection via the `eventCollector` option. See [guides/transactional-outbox-pattern.md](/patterns/transactional-outbox) for details.
