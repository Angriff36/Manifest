...
did real, good work but committed it badly. Your job is to land the *content*
cleanly and discard the *mechanics* of how it was committed. Git works normally
...
assistant) are correct in *content* but polluted: they swept two nested git
...
1. **Nested worktrees + build junk were committed.** Staging used `git add -A`
   from the repo root, which swallowed the *registered but nested* worktrees under
