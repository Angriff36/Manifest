...
(cd "$WT" && echo "Apply the minimal-fix skill (.claude/skills/minimal-fix/SKILL.md): fix exactly this and nothing else: $TARGET. Respect loop-constraints.md denylist. Run pnpm test / focused tests. Do not commit. Do not push." \
...
echo "Maker must not run its own checker."
