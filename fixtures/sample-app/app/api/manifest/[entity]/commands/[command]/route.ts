...
export async function POST(request: NextRequest, ctx: DispatcherContext) {
  try {
    ...
    if (!normalized.success) {
      ...
      const status =
        firstDiagnostic?.kind === 'policy_denial'
          ? 403
          : firstDiagnostic?.kind === 'guard_failure'
            ? 422
            : firstDiagnostic?.kind === 'constraint_block'
              ? 422
              : 400;
      return manifestErrorResponse(
        { error: normalized.error, diagnostics: normalized.diagnostics },
        status,
      );
    }
