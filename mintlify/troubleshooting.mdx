...
<AccordionGroup>
  ...
  <Accordion title="Read policies are not being enforced">
    ...
    **How to fix it**
    ...
    If you need a policy enforced during command execution, use the `execute` or `all` scope instead. Policies are entity-level declarations with the form `policy Name scope: expression`:
