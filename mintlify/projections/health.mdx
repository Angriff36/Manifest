...
<Note>
  Generated IR / non-memory store / outbox checks contain explicit `TODO` stubs and currently
  report healthy. Treat emitted checks as scaffolding until live probes are implemented in the
  generated (or post-edited) handler. Next.js and Express wrappers do not emit the core handler —
  generate `health.handler` (or provide an equivalent) at `handlerImportPath`.
</Note>
