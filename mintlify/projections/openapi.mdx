...
<Note>
Earlier feature notes list `includeAuth`, `includeConstraintErrors`, and the `info`/`servers`/`securitySchemes`/`security` options; the source additionally exposes `includeTenant`, `includePolicySecurity`, and `generatedAt`, all documented above. The `email` type maps to `string` with `format: email`, and `url`/`uri` are also handled even though older notes do not mention them.
</Note>
