...
<Note>
When a `signature` block is declared, the reference runtime verifies the HMAC over the exact received request bytes before the command is dispatched, comparing with `crypto.timingSafeEqual`. The provided signature may be bare hex or a `sha256=`/`sha512=`-prefixed hex (GitHub convention). A missing or invalid signature is rejected `401` and no command runs; an unresolvable `secret` context path or an unsupported `algorithm` is a fail-closed `500`. A webhook with **no** `signature` block is unauthenticated by design — declare one to require authentication. The full request pipeline (matching, idempotency, transform, dispatch statuses) is specified in `docs/spec/semantics.md` § "Webhooks (Inbound HTTP Triggers)".
</Note>
