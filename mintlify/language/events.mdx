...
~~**Correction (2026-07-15 — STALE, see below)** Source may declare
~~`store <Entity> in eventSourced { ... }` and the compiler accepts it into IR (`target: "eventSourced"`,
~~fixture `83-event-sourced.manifest`). There is **no** `EventSourcedStore` implementation in the
~~reference runtime — the kind is passthrough only. Do not document append-only / snapshot / replay
~~behavior as shipped. Use a real store target (`memory`, adapter-backed, etc.) until an
