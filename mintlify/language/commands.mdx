...
<Steps>
  <Step title="Process parameters">
    Apply each declared parameter's `defaultValue` to any omitted argument, then fail closed with a `parameterFailure` (`MISSING_REQUIRED_PARAMETER`) when a `required` parameter with no default is absent. An explicit `undefined` is treated as absent; `null` counts as supplied.
  </Step>
  <Step title="Build evaluation context">
    Bind `self` and `this` to the current entity instance, inject command parameters (with defaults already applied), and attach the runtime context (`user`, `context.*`).
  </Step>
