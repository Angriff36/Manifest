...
Intended for sending the expression result to an external message broker or outbox adapter. In the default (no-adapter) runtime, `publish` behaves identically to `emit` — it logs an anonymous `action_event` to the event log with the evaluated value as the payload. There is no per-action call to an external bus; the event bus publish happens post-command for all declared `emits`. In deterministic mode, throws `ManifestEffectBoundaryError`.

### `persist`

Intended for invoking a persistence adapter with the expression result. In the default runtime, `persist` is a no-op — it evaluates the expression but performs no store write. Real persistence happens only via `mutate` and `compute` (which call `updateInstance`). In deterministic mode, throws `ManifestEffectBoundaryError`.

### `effect`

Intended for invoking a side-effect adapter. In the default runtime, `effect` evaluates the expression but invokes no side-effect hook and discards the result. In deterministic mode, throws `ManifestEffectBoundaryError`.

<Warning>
`persist` and `effect` perform no meaningful work without a configured adapter. `publish` always logs an anonymous `action_event` to the local event log but does not call an external message bus unless one is wired. When `deterministicMode` is `true`, calling any of them throws `ManifestEffectBoundaryError` — this is by design to enforce the effect boundary contract.
</Warning>
