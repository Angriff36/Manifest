...
<Warning>
Before `autoNow` existed, a `= now()` default compiled to **no** default and the field was null-filled at runtime, breaking non-null store columns. An **unsupported** call-expression default — e.g. `= uuid()` — would still be silently dropped, so the compiler now emits a **warning** for it (listing the supported call defaults `now()`, `today()`). Separately, a `create` command that leaves a non-null, default-less property unset is **warned** about when the runtime would null-fill that field — the types that fill with `null` rather than a zero value: `datetime`, `date`, `time`, `enum`, and custom/value-object types. Zero-filling types (`string`, `number`, `boolean`, `list`/`array`, `map`) are not flagged, nor are runtime/store-managed fields (`id`, composite keys, relationship FKs, the tenant property, version fields, auto timestamps). Both are warnings (not hard errors, since the runtime merges caller input on create) and surface in `manifest compile` and the LSP.
</Warning>
