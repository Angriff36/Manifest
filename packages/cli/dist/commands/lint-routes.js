 ...
 */
export async function loadLintRoutesConfig(cwd) {
    ...
    for (const configFile of configPaths) {
        ...
        try {
            ...
            if (parsed?.lintRoutes && typeof parsed.lintRoutes === 'object') {
                const lintConfig = parsed.lintRoutes;
                return {
 ...
 */
function buildRoutePattern(prefixes) {
 ...
 */
function isCommentOrGenerated(line) {
 ...
 */
export function scanFileForRoutes(content, filePath, config) {
    const violations = [];
 ...
 */
export async function scanDirectories(cwd, config) {
    const violations = [];
    let filesScanned = 0;
 ...
 */
export async function lintRoutesCommand(options = {}) {
...
//# sourceMappingURL=lint-routes.js.map