...
import { auditRouteFileContent, isInCommandsNamespace, isExempted, extractCommandFromPath, extractEntitySegmentFromPath, hasCommandManifestBacking, loadCommandsManifest, loadExemptions, AuditUsageError, } from './audit-routes';
const OPTIONS = {
...
`;
                const result = auditRouteFileContent(content, 'app/api/kitchen/tasks/commands/create/route.ts', OPTIONS, makeOwnership({ commandsManifest: [], manifestExplicitlyProvided: true }));
                expect(result.findings.some((f) => f.code === 'COMMAND_ROUTE_ORPHAN')).toBe(true);
...
`;
                const result = auditRouteFileContent(content, 'app/api/kitchen/tasks/commands/create/route.ts', OPTIONS, makeOwnership({ commandsManifest: [], manifestExplicitlyProvided: false }));
                expect(result.findings.some((f) => f.code === 'COMMAND_ROUTE_ORPHAN')).toBe(false);
            });
        });
        // ------------------------------------------------------------------
