...
const INIT_QUESTIONS = [
    {
        ...
        default: '**/*.manifest',
        validate: (input) => input.length > 0 || 'Pattern cannot be empty',
    },
    {
        type: 'input',
        name: 'outputDir',
    ...
    },
    {
        ...
        default: 'nextjs',
        when: (answers) => answers.enableCodegen,
        validate: (input) => input.length > 0 || 'Target cannot be empty',
    },
    {
        type: 'input',
        name: 'codeOutputDir',
 ...
 */
function showPostInit(answers, config) {
    console.log('');
...
export async function initCommand(options = {}) {
    try {
        ...
        // Ask questions
        const answers = await inquirer.prompt(INIT_QUESTIONS);
        // Create config
