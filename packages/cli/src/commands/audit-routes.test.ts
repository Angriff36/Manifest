...
`;
  ...
  describe("isInCommandsNamespace", () => {
    it("detects /commands/ in path", () => {
      expect(
        isInCommandsNamespace("app/api/kitchen/tasks/commands/create/route.ts")
      ).toBe(true);
      expect(
        isInCommandsNamespace(
          "apps/api/app/api/crm/clients/commands/update/route.ts"
        )
      ).toBe(true);
    });

    it("rejects paths without /commands/", () => {
      expect(isInCommandsNamespace("app/api/kitchen/tasks/route.ts")).toBe(
        false
      );
    ...
    it("handles Windows-style backslash paths", () => {
      expect(
        isInCommandsNamespace(
          "app\\api\\kitchen\\tasks\\commands\\create\\route.ts"
        )
      ).toBe(true);
    });
  });
  ...
  describe("extractCommandFromPath", () => {
    it("extracts command name from commands namespace path", () => {
      expect(
        extractCommandFromPath("app/api/kitchen/tasks/commands/create/route.ts")
      ).toBe("create");
      expect(
        extractCommandFromPath("app/api/crm/clients/commands/archive/route.ts")
      ).toBe("archive");
    });

    it("returns null for non-commands paths", () => {
      expect(
        extractCommandFromPath("app/api/kitchen/tasks/route.ts")
      ).toBeNull();
    });
  ...
  describe("extractEntitySegmentFromPath", () => {
    it("extracts entity segment before /commands/", () => {
      ...
      expect(
        extractEntitySegmentFromPath(
          "app/api/crm/clients/commands/update/route.ts"
        )
      ).toBe("clients");
    });
    ...
    it("returns null for non-commands paths", () => {
      expect(
        extractEntitySegmentFromPath("app/api/kitchen/tasks/route.ts")
      ).toBeNull();
    });
  });
  ...
  describe("isExempted", () => {
    ...
    it("returns true for exempted path + method", () => {
      expect(
        isExempted(
          "/root/app/api/webhooks/clerk/route.ts",
          "POST",
          exemptions,
          "/root"
        )
    ...
    it("returns false for non-exempted path", () => {
      expect(
        isExempted(
          "/root/app/api/accounting/route.ts",
          "POST",
          exemptions,
          "/root"
        )
    ...
    it("refuses to match files outside root via .. traversal", () => {
      expect(
        isExempted(
          "/other/app/api/webhooks/clerk/route.ts",
          "POST",
          exemptions,
          "/root"
        )
    ...
    it("handles Windows backslash paths against forward-slash exemptions", () => {
      ...
      expect(
        isExempted(
          "/root/app/api/timecards/bulk/route.ts",
          "POST",
          exemptions,
          "/root"
        )
  ...
  describe("hasCommandManifestBacking", () => {
    ...
    it("matches by command name only — entity naming conventions differ between IR and filesystem", () => {
      ...
      expect(
        hasCommandManifestBacking(
          "app/api/crm/clients/commands/archive/route.ts",
          manifest
        )
      ).toBe(true);
    });
    ...
    it("returns false for non-commands path", () => {
      expect(
        hasCommandManifestBacking("app/api/kitchen/tasks/route.ts", manifest)
      ).toBe(false);
    });
  ...
  describe("ownership enforcement", () => {
    ...
    function makeOwnership(
      overrides?: Partial<OwnershipContext>
    ): OwnershipContext {
      return {
        commandsManifest,
        exemptions,
...
`;
        ...
        const orphanWarning = warningResult.findings.find(
          (f) => f.code === "COMMAND_ROUTE_ORPHAN"
        );
        ...
        const orphanError = errorResult.findings.find(
          (f) => f.code === "COMMAND_ROUTE_ORPHAN"
        );
...
`;
  ...
  describe("loadCommandsManifest", () => {
    ...
    it("returns empty array for non-existent file (ENOENT)", async () => {
      const result = await loadCommandsManifest(
    ...
    it("throws AuditUsageError on malformed JSON", async () => {
      ...
      await expect(loadCommandsManifest(filePath)).rejects.toThrow(
        "not valid JSON"
      );
      await expect(loadCommandsManifest(filePath)).rejects.toThrow(
        AuditUsageError
  ...
  describe("loadExemptions", () => {
    ...
    it("throws AuditUsageError on non-array JSON", async () => {
      ...
      await expect(loadExemptions(filePath)).rejects.toThrow(
        "must be an array"
      );
      await expect(loadExemptions(filePath)).rejects.toThrow(AuditUsageError);
      await rm(testDir, { recursive: true, force: true });
...
`;
      ...
      const ownershipErrors = allErrors.filter((f) =>
        OWNERSHIP_RULE_CODES.has(f.code)
      );
      const nonOwnershipErrors = allErrors.filter(
        (f) => !OWNERSHIP_RULE_CODES.has(f.code)
      );
