...
`;
  ...
  describe('isExempted', () => {
    const exemptions: RouteExemption[] = [
      { path: 'app/api/webhooks/clerk/route.ts', methods: ['POST'], reason: 'Auth callback' },
      { path: 'app/api/timecards/bulk/route.ts', methods: ['POST'], reason: 'Legacy bulk' },
    ];
  ...
  describe('hasCommandManifestBacking', () => {
    const manifest: CommandsManifestEntry[] = [
      ...
      { entity: 'KitchenTask', command: 'update', commandId: 'KitchenTask.update' },
      { entity: 'CrmClient', command: 'archive', commandId: 'CrmClient.archive' },
    ];
...
`;
        const result = auditRouteFileContent(
          ...
          'app/api/kitchen/tasks/commands/create/route.ts',
          OPTIONS,
          makeOwnership({ commandsManifest: [], manifestExplicitlyProvided: true }),
        );
        expect(result.findings.some((f) => f.code === 'COMMAND_ROUTE_ORPHAN')).toBe(true);
...
`;
        const result = auditRouteFileContent(
          ...
          'app/api/kitchen/tasks/commands/create/route.ts',
          OPTIONS,
          makeOwnership({ commandsManifest: [], manifestExplicitlyProvided: false }),
        );
        expect(result.findings.some((f) => f.code === 'COMMAND_ROUTE_ORPHAN')).toBe(false);
      });
    });
...
`;
        const result = auditRouteFileContent(
          ...
          makeOwnership({
            exemptions: [
              { path: 'app/api/kitchen/tasks/commands/create/route.ts', methods: ['POST'], reason: 'test' },
            ],
          }),
        );
        expect(result.findings.some((f) => f.code === 'COMMAND_ROUTE_MISSING_RUNTIME_CALL')).toBe(true);
...
`;
        const mixedExemptions: RouteExemption[] = [
          { path: 'app/api/timecards/route.ts', methods: ['POST'], reason: 'Legacy POST only' },
        ];
        const result = auditRouteFileContent(
          content,
          'app/api/timecards/route.ts',
...
`;
  ...
  describe('loadExemptions', () => {
    ...
    it('parses valid exemptions file', async () => {
      ...
      await writeFile(filePath, JSON.stringify([
        { path: 'app/api/webhooks/route.ts', methods: ['POST'], reason: 'Webhook' },
      ]));
    ...
    it('filters out entries with missing required fields', async () => {
      ...
      const filePath = join(testDir, 'mixed.json');
      await writeFile(filePath, JSON.stringify([
        { path: 'app/api/webhooks/route.ts', methods: ['POST'], reason: 'Valid' },
