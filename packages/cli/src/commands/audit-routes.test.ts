...
import type { CommandsManifestEntry, OwnershipContext, RouteExemption } from './audit-routes';
import {
  AuditUsageError,
  auditRouteFileContent,
  extractCommandFromPath,
  extractEntitySegmentFromPath,
  hasCommandManifestBacking,
  isExempted,
  isInCommandsNamespace,
...
`;
    ...
    const result = auditRouteFileContent(content, 'app/api/items/route.ts', OPTIONS);
    expect(result.findings.some((f) => f.code === 'READ_LOCATION_REFERENCE_WITHOUT_FILTER')).toBe(
      true,
    );
  });
...
`;
  ...
  describe('isInCommandsNamespace', () => {
    it('detects /commands/ in path', () => {
      ...
      expect(isInCommandsNamespace('apps/api/app/api/crm/clients/commands/update/route.ts')).toBe(
        true,
      );
    });
    ...
    it('handles Windows-style backslash paths', () => {
      expect(isInCommandsNamespace('app\\api\\kitchen\\tasks\\commands\\create\\route.ts')).toBe(
        true,
      );
    });
  });
  ...
  describe('extractCommandFromPath', () => {
    it('extracts command name from commands namespace path', () => {
      expect(extractCommandFromPath('app/api/kitchen/tasks/commands/create/route.ts')).toBe(
        'create',
      );
      expect(extractCommandFromPath('app/api/crm/clients/commands/archive/route.ts')).toBe(
        'archive',
      );
    });
    ...
    it('handles Windows paths', () => {
      expect(extractCommandFromPath('app\\api\\kitchen\\tasks\\commands\\create\\route.ts')).toBe(
        'create',
      );
    });
  });
  ...
  describe('extractEntitySegmentFromPath', () => {
    it('extracts entity segment before /commands/', () => {
      expect(extractEntitySegmentFromPath('app/api/kitchen/tasks/commands/create/route.ts')).toBe(
        'tasks',
      );
      expect(extractEntitySegmentFromPath('app/api/crm/clients/commands/update/route.ts')).toBe(
        'clients',
      );
    });
  ...
  describe('isExempted', () => {
    ...
    it('returns true for exempted path + method', () => {
      expect(isExempted('/root/app/api/webhooks/clerk/route.ts', 'POST', exemptions, '/root')).toBe(
        true,
      );
    });
    ...
    it('returns false for exempted path but wrong method', () => {
      expect(
        isExempted('/root/app/api/webhooks/clerk/route.ts', 'DELETE', exemptions, '/root'),
      ).toBe(false);
    });

    it('returns false for non-exempted path', () => {
      expect(isExempted('/root/app/api/accounting/route.ts', 'POST', exemptions, '/root')).toBe(
        false,
      );
    });
    ...
    it('refuses to match files outside root via .. traversal', () => {
      expect(
        isExempted('/other/app/api/webhooks/clerk/route.ts', 'POST', exemptions, '/root'),
      ).toBe(false);
    });
    ...
    it('refuses to match when relative path resolves to absolute', () => {
      ...
      expect(
        isExempted('D:\\app\\api\\webhooks\\clerk\\route.ts', 'POST', exemptions, 'C:\\root'),
      ).toBe(false);
    });
    ...
    it('handles Windows backslash paths against forward-slash exemptions', () => {
      ...
      expect(isExempted('/root/app/api/timecards/bulk/route.ts', 'POST', exemptions, '/root')).toBe(
        true,
      );
    });
  });
  ...
  describe('hasCommandManifestBacking', () => {
    const manifest: CommandsManifestEntry[] = [
      {
        entity: 'KitchenTask',
        command: 'create',
    ...
    it('returns true when command name matches', () => {
      expect(
        hasCommandManifestBacking('app/api/kitchen/tasks/commands/create/route.ts', manifest),
      ).toBe(true);
    });

    it('returns false when command name has no match', () => {
      expect(
        hasCommandManifestBacking('app/api/kitchen/tasks/commands/foo/route.ts', manifest),
      ).toBe(false);
    });
    ...
    it('matches by command name only — entity naming conventions differ between IR and filesystem', () => {
      ...
      expect(
        hasCommandManifestBacking('app/api/crm/clients/commands/archive/route.ts', manifest),
      ).toBe(true);
    });
    ...
    it('matches kebab-case filesystem paths against camelCase IR commands', () => {
      const multiWordManifest: CommandsManifestEntry[] = [
        {
          entity: 'Station',
          command: 'assignTask',
      ...
      expect(
        hasCommandManifestBacking(
          'app/api/kitchen/stations/commands/assign-task/route.ts',
          multiWordManifest,
        ),
      ).toBe(true);
      expect(
        hasCommandManifestBacking(
          'app/api/timecards/entries/commands/clock-in/route.ts',
          multiWordManifest,
        ),
      ).toBe(true);
      expect(
        hasCommandManifestBacking(
          'app/api/kitchen/prep-lists/commands/create-from-seed/route.ts',
          multiWordManifest,
        ),
      ).toBe(true);
      expect(
        hasCommandManifestBacking(
          'app/api/kitchen/prep-list-items/commands/update-prep-notes/route.ts',
          multiWordManifest,
        ),
      ).toBe(true);
      ...
      expect(
        hasCommandManifestBacking(
          'app/api/kitchen/stations/commands/assignTask/route.ts',
          multiWordManifest,
        ),
      ).toBe(true);
      ...
      expect(
        hasCommandManifestBacking(
          'app/api/kitchen/stations/commands/nonexistent-task/route.ts',
          multiWordManifest,
        ),
      ).toBe(false);
    });
  ...
  describe('ownership enforcement', () => {
    const commandsManifest: CommandsManifestEntry[] = [
      {
        entity: 'KitchenTask',
        command: 'create',
        commandId: 'KitchenTask.create',
    ...
    function makeOwnership(overrides?: Partial<OwnershipContext>): OwnershipContext {
      return {
        commandsManifest,
        exemptions,
        root: '',
...
`;
        const result = auditRouteFileContent(
          content,
          'app/api/kitchen/tasks/commands/foo/route.ts',
          OPTIONS,
          makeOwnership(),
        );
...
`;
        const result = auditRouteFileContent(
          content,
          'app/api/kitchen/tasks/commands/create/route.ts',
          OPTIONS,
          makeOwnership(),
        );
...
`;
        const result = auditRouteFileContent(
          ...
          OPTIONS,
          makeOwnership({ exemptions: [] }),
        );
        expect(result.findings.some((f) => f.code === 'WRITE_OUTSIDE_COMMANDS_NAMESPACE')).toBe(
          true,
        );
      });
...
`;
        ...
        expect(result.findings.some((f) => f.code === 'COMMAND_ROUTE_MISSING_RUNTIME_CALL')).toBe(
          true,
        );
      });
...
`;
        const result = auditRouteFileContent(
          ...
          'app/api/timecards/route.ts',
          OPTIONS,
          makeOwnership({ enforceOwnership: false, exemptions: [] }),
        );
...
`;
        const result = auditRouteFileContent(
          ...
          'app/api/timecards/route.ts',
          OPTIONS,
          makeOwnership({ enforceOwnership: true, exemptions: [] }),
        );
        const violation = result.findings.find(
          (f) => f.code === 'WRITE_OUTSIDE_COMMANDS_NAMESPACE',
        );
...
`;
        const mixedExemptions: RouteExemption[] = [
          {
        ...
        const result = auditRouteFileContent(
          ...
          'app/api/timecards/route.ts',
          OPTIONS,
          makeOwnership({ exemptions: mixedExemptions, root: '' }),
        );
        const violations = result.findings.filter(
          (f) => f.code === 'WRITE_OUTSIDE_COMMANDS_NAMESPACE',
        );
...
`;
        const warningResult = auditRouteFileContent(
          content,
          'app/api/kitchen/tasks/commands/foo/route.ts',
          OPTIONS,
          makeOwnership({ enforceOwnership: false }),
        );
        ...
        const errorResult = auditRouteFileContent(
          content,
          'app/api/kitchen/tasks/commands/foo/route.ts',
          OPTIONS,
          makeOwnership({ enforceOwnership: true }),
        );
...
`;
        ...
        const missingWarning = warningResult.findings.find(
        ...
        const missingError = errorResult.findings.find(
          (f) => f.code === 'COMMAND_ROUTE_MISSING_RUNTIME_CALL',
        );
  ...
  describe('--strict gate semantics', () => {
    it('OWNERSHIP_RULE_CODES contains exactly the three ownership rules', () => {
      expect(OWNERSHIP_RULE_CODES).toEqual(
        new Set([
          'WRITE_OUTSIDE_COMMANDS_NAMESPACE',
          'COMMAND_ROUTE_MISSING_RUNTIME_CALL',
...
`;
      ...
      const ownershipErrors = result.findings.filter(
        (f) => OWNERSHIP_RULE_CODES.has(f.code) && f.severity === 'error',
      );
