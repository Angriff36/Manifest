...
export const PROJECTION_META_SCHEMAS = {
  defaults: {
    ...
    additionalProperties: true,
    description:
      'Shared options merged under every named projection\'s `options` (per-projection keys win). ' +
...
export function buildProjectionsProperties(currentSchema, projectionNames) {
  ...
  const meta = {};
  for (const metaKey of Object.keys(PROJECTION_META_SCHEMAS).sort()) {
    meta[metaKey] = existing[metaKey] ?? PROJECTION_META_SCHEMAS[metaKey];
  }
