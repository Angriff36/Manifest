...
- docs: record indexed query storage proof
- fix: align Convex indexed query arguments with stored types
...
- docs: record transactional event handler proof
- feat: support transactional Convex event handlers
...
- fix(convex): materialize child computeds for parent aggregates
...
- fix(convex): seed optional command defaults in create drafts
...
- fix: hydrate relations referenced by emit payloads
...
- fix(loop): match capsule-pro GLM/MiniMax routing
...
- fix compiler cron steps and reaction payloads
...
- Wiring remediate: do not treat named props types (e.g. `TaskCardProps`) as the value type of a destructured binding — only inline property types prove nullability/compatibility
...
- Wiring remediate: scope `replace-object-property-value` to the target property so empty-string sentinels (`""`) are not replaced at the first file-wide occurrence
...
- fix(cli): rename seed template --version to --pack-version
...
- Wiring remediate: refuse to export sync helpers from `"use server"` modules (Next.js requires Server Action exports to be async); expand-partial expects full-body builders in a non-server-action module
...
- Wiring inspect: `runManifestCommand` non-literal `body:` expressions (helpers, identifiers, ternaries) are isolated as unresolved payload shapes instead of falling back to outer option keys — stops false `missing_required_input` for Capsule `Event.update` via `buildUpdateBody(...)`

## [3.4.2] - 2026-07-09

### Fixed

- Wiring inspect: object-literal field extraction recognizes ES property shorthand (`{ dropOff, bringHot }`) so valid generated-client payloads are not reported as `missing_required_input` (e.g. Capsule `RecipeVersion.setPackaging`)
