...
1. Prisma: `date → DateTime` already exists today *without* `@db.Date`; `time` follows that precedent (plain `DateTime`). `duration → Float` as specced.
...
- [ ] **Step 2: Regenerate expected IR** — `pnpm run conformance:regen`, then inspect `expected/92-date-time-types.ir.json`: property types must read `{"name":"date",...}` etc.; the `date`/`time` *named* properties must read `{"name":"string"}`/`{"name":"number"}`. UTF-8 without BOM, no random values.
  ...
  "testCases": [
    {
      ...
      "createInstance": {
        ...
        "data": {
          ...
          "day": "2026-06-09", "startsAt": "09:30:00",
          "due": 1000000000000, "length": 3600000,
          "date": "free-form name regression", "time": 42
        }
      },
      "expectedInstance": {
        "id": "meeting-1", "title": "Standup",
