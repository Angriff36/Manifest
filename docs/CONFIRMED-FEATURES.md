...
- config-vNext: G0+G1+G5+G2(`failOn`+`rules`)+G3(`mergeIntegrity`)+G4(`provenance`)+G7(`runtime` generation slice)+G8(`hooks.lifecycle`)+G9(`plugins.order`/`capabilities`)+G10(`ci-gate`) shipped (see `config.ts` comments + matrix §1). ~~G5/G2/G10 still unbuilt~~ **Correction (2026-07-15):** those three shipped. **Correction (2026-07-22):** G9 shipped (`plugin-order.ts` + loader `loadOrder`/`declaredCapabilities`). **Correction (2026-07-22):** G8 shipped (`lifecycle-hooks.ts` + compile `beforeCompile` / generate `afterGenerate`). **Correction (2026-07-22):** G3 shipped (`merge-integrity.ts` + multi-compiler `error`\|`lastWins`). **Correction (2026-07-22):** G4 shipped (`provenance-config.ts` + deterministic `compiledAt` / lockfile / `failIfStale`; `gitSha` not written yet). ~~**Correction (2026-07-22):** G7 shipped … concurrency deferred.~~ **Correction (2026-07-22):** G7 shipped (`runtime-config.ts` + nextjs dispatcher `executionMode` / web factory determinism + `stores`→`runtimeConfigImport` + `defaultContext` + `concurrency.maxParallelCommands`→`RuntimeOptions.maxParallelCommands`). **Correction (2026-07-22):** G2 `validation.rules` shipped (`missing-policy` / `unused-entity` / `orphan-relationship`; `requireDescriptions` deferred). Remaining open: G2 `requireDescriptions` — see `docs/internal/proposals/config/manifest-config-vnext.md`.
