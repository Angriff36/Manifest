...
> **Correction (2026-07-15) @RYANSIGNED:** This document is a **historical**
> planning snapshot from the **v0.3.8** era. It is **not** current product
> status. Package pin SoT is root `package.json` = **3.6.41**. Do not treat
> “95% compliant”, “UNFIXED” critical issues, or line-number citations below
> as live truth without re-verifying against `src/manifest/` and
> `docs/spec/`. Prefer `docs/guides/migration/vnext.md` + Tier A specs for
...
~~Manifest v0.3.8 is **95% compliant** with the IR v1 specification, workflow addendum, and all documentation artifacts.~~ The historical snapshot claimed high compliance and listed open issues below. For current conformance evidence, run `pnpm test` against `src/manifest/conformance/*`. The implementation (then and now) correctly follows:
...
**Severity**: CRITICAL
**Status**: **FIXED** — `CommandResult.concurrencyConflict` is populated on version mismatch (`runtime-engine.ts`)
...
export class IRCache {
  ...
  get(contentHash: string): unknown | null {
    ...
    if (this.clock() - entry.timestamp > this.maxAge) {
      // Use clock
      this.cache.delete(contentHash);
      return null;
    }
