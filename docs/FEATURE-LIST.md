...
| `convex` | `convex.schema`, `convex.queries`, `convex.mutations`, `convex.crons`, `convex.http`, `convex.sagas`, `convex.computed`, `convex.react` | yes | 29 supported / 7 partial / 5 unsupported |
...
| 6. Projections — every registered target | convex | `PARTIAL` | mirror of COMPLIANCE_MATRIX §6 — relationship read hydration + saga shared-input + hard-delete onDelete cascade/restrict Supported; onUpdate/setNull/setDefault/composite still Partial; read rateLimit/async/action-kind/retry/approvals REJECTED_LOUD; other Partial rows (trustedSource/realtime/…) remain |
