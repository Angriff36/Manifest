...
This is not just a database URL setting. Each durable entity needs a store
binding containing its adapter and table/model mapping. Only `memory` and
browser `localStorage` can use the generated factory without config. Other
targets fail and name the unbound entities. See the
[complete runtime store example](./config/manifest.config.md#complete-postgresql-runtime-companion-example).
