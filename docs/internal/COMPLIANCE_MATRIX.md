...
| [~]    | convex                | PARTIAL                        | core generate + auth seam; 2026-07-15: bare-emit schema/result payloads (not `{}`), seed persistent-only + typed/deduped args, assembly verify event+seed coherence (tests green; publish+SHA pending) |
