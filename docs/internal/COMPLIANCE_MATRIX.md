...
Pin / consumption evidence: Builder `package.json` currently pins `@angriff36/manifest@3.6.6` (verified 2026-07-15; Manifest SoT version is `package.json` — verify before asserting). Detail rows: Builder matrix.
...
| [~]    | convex                | PARTIAL                        | core generate + auth seam; 2026-07-15: G7 `self.id`→`_id`/`docId`, `previous*`→pre-update `doc`, react `../../convex/_generated/api`, auth seam → public query + useQuery, seed fill/no empty `{}` (tests green; publish+SHA pending for FULLY_IMPLEMENTED rows) |
