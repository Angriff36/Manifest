...
| [~]    | convex                | PARTIAL                        | core generate + auth seam; 2026-07-15: bare-emit schema/result payloads, seed/assembly fixes, read-policy query enforcement, and `encryptionImport` store-boundary encryption (focused tests green; commit proof pending) |
...
| [~]    | Convex read/`all` policy query enforcement     | CLAIMED_NEEDS_PROOF   | `read-policies.ts`, `functions.ts`, focused query tests; commit SHA pending            |
| [~]    | Convex encrypted-property store boundary       | CLAIMED_NEEDS_PROOF   | `encryptionImport`, runtime-compatible envelope, focused read/write tests; SHA pending |
