...
- **Rate limiting is enforced by the runtime engine before command execution.** The default `MemoryRateLimitStore` is in-memory and resets on process restart. A **durable `PostgresRateLimitStore`** adapter is shipped (`@angriff36/manifest/rate-limit/postgres`) that uses `SELECT … FOR UPDATE` to provide coherent shared-bucket state across concurrent process instances. For multi-process deployments, configure `RuntimeOptions.rateLimitStore` with the Postgres adapter. (RYANSIGNED 2026-07-22)
