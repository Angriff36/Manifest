...
const severityBg = (severity: 'ok' | 'warn' | 'block', passed: boolean) => {
  ...
  switch (severity) {
    ...
    case 'block':
      return 'bg-rose-900/20 border-rose-800/50';
  }
};
...
const severityLabel = (severity: 'ok' | 'warn' | 'block') => {
  switch (severity) {
    case 'ok':
      return { text: 'OK', className: 'text-gray-400 bg-gray-800' };
    case 'warn':
      return { text: 'WARN', className: 'text-amber-400 bg-amber-900/40' };
    case 'block':
      return { text: 'BLOCK', className: 'text-rose-400 bg-rose-900/40' };
  }
};
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-500">
                        {entity.constraints.length} constraint
                        {entity.constraints.length !== 1 ? 's' : ''}
                      </div>
                    </div>
                  </div>
                </button>
                  ...
                  {selectedEntity.properties.map((prop) => {
                    ...
                    return (
                      <div key={prop.name} className="flex items-center gap-2">
                        <label
                          className="w-32 text-xs text-gray-400 truncate flex-shrink-0"
                          title={prop.name}
                        >
                          {prop.name}
                        ...
                        {prop.type.name === 'boolean' ? (
                          ...
                          <input
                            type={
                              prop.type.name === 'number' || prop.type.name === 'decimal'
                                ? 'number'
                                : 'text'
                            }
                ...
                <div className="space-y-2">
                  ...
                  {outcomes.length === 0 ? (
                    ...
                    outcomes.map((outcome) => {
                      ...
                      const constraint = selectedEntity.constraints.find(
                        (c) => c.code === outcome.code,
                      );
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            ...
                            <span
                              className={`text-xs px-1.5 py-0.5 rounded font-mono ${sev.className}`}
                            >
                              {sev.text}
                            </span>
                            <span
                                ...
                                <div>
                                  ...
                                  <div className="space-y-1">
                                    {outcome.resolved.map((rv, i) => (
                                      <div
                                        key={i}
                                        className="flex items-center gap-2 text-xs font-mono bg-gray-950 px-2 py-1 rounded border border-gray-800"
                                      >
                ...
                <div className="space-y-2">
                  ...
                  {selectedEntity.constraints.map((constraint) => {
                    ...
                    return (
                      <div
                        key={constraint.code}
                        className="px-3 py-2 bg-gray-900/30 rounded border border-gray-800 flex items-center gap-2"
                      >
              ...
              {entities.length === 0 ? (
                <>
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    Add constraint blocks to your entities to test them here.
                  </p>
                </>
              ) : (
                <>
