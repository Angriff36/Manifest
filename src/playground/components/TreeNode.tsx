...
export function TreeNode({ label, value, depth = 0 }: { label: string; value: unknown; depth?: number }) {
  ...
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
    const colorClass = typeof value === 'string' ? 'text-amber-400'
  ...
  if (Array.isArray(value)) {
    ...
    return (
      <div>
        ...
        {open && value.map((item, i) => <TreeNode key={i} label={`[${i}]`} value={item} depth={depth + 1} />)}
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        >
          ...
          {(value as any).type && <span className="text-emerald-400 text-xs ml-1">{(value as any).type}</span>}
        </button>
