...
import { isBlankCell, type SeedPack } from './types.js';
import {
  findEntity,
  findProperty,
  findRelationship,
...
export function validateSeedPack(
  ...
  for (const table of pack.tables) {
    ...
    seedKeysByEntity.set(table.entity, seen);
  }

  // FK seedKey references
  for (const table of pack.tables) {
    const entity = findEntity(ir, table.entity);
    if (!entity) continue;
    for (const row of table.rows) {
      for (const col of table.columns) {
        ...
        if (!targetKeys) {
          // Target entity not in pack — soft warning if IR has it, error if required later
  ...
  // Soft drift: unused IR entities are informational only (never errors).
