...
export function classifyBreakingChanges(
  ...
  const consumerImpact: ConsumerImpactSummary = {
    commands: [...new Set(allImpact.filter((i) => i.startsWith('command:')))].sort((a, b) =>
      a.localeCompare(b),
    ),
    routes: [...new Set(allImpact.filter((i) => i.startsWith('route:')))].sort((a, b) =>
      a.localeCompare(b),
    ),
    projections: [...new Set(allImpact.filter((i) => i.startsWith('projection:')))].sort((a, b) =>
      a.localeCompare(b),
    ),
  };
