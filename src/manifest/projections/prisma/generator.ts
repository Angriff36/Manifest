 ...
 *
 * Consumes Manifest IR + projection config and emits Prisma schema artifact(s).
 * Default: one combined file. With `multiSchema.splitFiles`, root + per-schema
 * partitions (see `split-files.ts`).
 *
...
import {
  buildSplitPrismaArtifacts,
  resolveSplitFilesPlan,
  type SchemaTaggedBlock,
} from './split-files.js';
import {
...
function emitPrismaConfigTs(envVar = 'DATABASE_URL', schemaDir?: string): string {
  return [
...
export class PrismaProjection implements ProjectionTarget {
  ...
  generate(ir: IR, request: ProjectionRequest): ProjectionResult {
    ...
    const taggedModels: SchemaTaggedBlock[] = [];
    for (const entity of toEmit) {
      const { lines, diagnostics: modelDiags } = emitModel(entity, ir, options, context);
      diagnostics.push(...modelDiags);
      taggedModels.push({
        schemaName: resolveSchemaName(entity, options) ?? 'public',
        code: lines.join('\n'),
      });
    }

    const taggedEnums: SchemaTaggedBlock[] = enumsToEmit.map((e) => ({
      schemaName: resolveEnumSchemaName(e, options) ?? 'public',
      code: emitEnum(e, options),
    }));
    ...
    const { plan: splitPlan, diagnostics: splitDiags } = resolveSplitFilesPlan(options);
    ...
    if (splitPlan) {
      ...
      const bodyBlocks = [...taggedModels.map((b) => b.code), ...taggedEnums.map((b) => b.code)];
      const code =
        bodyBlocks.length > 0
          ? headerStr + '\n' + bodyBlocks.join('\n\n') + '\n'
    ...
    if (options.provider) {
      artifacts.push({
        ...
        code: emitPrismaConfigTs(options.urlEnvVar ?? 'DATABASE_URL', splitPlan?.dir),
      });
    }
