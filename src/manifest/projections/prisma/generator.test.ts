...
describe('PrismaProjection — dbAttributes config', () => {
  ...
  it('suppresses the empty-string default on uuid-typed fields (unset-FK sentinel)', () => {
    ...
    expect(code).toMatch(/^\s+parentId String\? @db\.Uuid$/m);
    expect(code).not.toMatch(/parentId String\? @db\.Uuid @default/);
    // ordinary string fields keep their empty-string default.
