...
describe('ZodProjection', () => {
  ...
  describe('zod.entity surface', () => {
    ...
    it('filters to specific entity when request.entity is set', () => {
      ...
      const result = projection.generate(ir, { surface: 'zod.entity', entity: 'Recipe' });
      expect(result.artifacts).toHaveLength(1);
      expect(result.artifacts[0].id).toBe('zod.entity.Recipe');
    });
    ...
    it('returns error for missing entity', () => {
      ...
      const result = projection.generate(ir, { surface: 'zod.entity', entity: 'NonExistent' });
  ...
  describe('type mapping', () => {
    it('maps all basic IR types to Zod expressions', () => {
      const properties = [
        { name: 'str', type: { name: 'string', nullable: false }, modifiers: [] },
        { name: 'txt', type: { name: 'text', nullable: false }, modifiers: [] },
        { name: 'bool', type: { name: 'boolean', nullable: false }, modifiers: [] },
        { name: 'b', type: { name: 'bool', nullable: false }, modifiers: [] },
        { name: 'num', type: { name: 'number', nullable: false }, modifiers: [] },
        { name: 'flt', type: { name: 'float', nullable: false }, modifiers: [] },
        { name: 'dec', type: { name: 'decimal', nullable: false }, modifiers: [] },
        { name: 'mon', type: { name: 'money', nullable: false }, modifiers: [] },
        { name: 'int', type: { name: 'int', nullable: false }, modifiers: [] },
        { name: 'intg', type: { name: 'integer', nullable: false }, modifiers: [] },
        { name: 'big', type: { name: 'bigint', nullable: false }, modifiers: [] },
        { name: 'dt', type: { name: 'date', nullable: false }, modifiers: [] },
        { name: 'dttm', type: { name: 'datetime', nullable: false }, modifiers: [] },
        ...
        { name: 'byt', type: { name: 'bytes', nullable: false }, modifiers: [] },
        { name: 'obj', type: { name: 'object', nullable: false }, modifiers: [] },
      ];
    ...
    it('maps nested array types', () => {
      const ir = makeMinimalIR({
        entities: [bareEntity('NestedArray', [
          {
            name: 'matrix',
            type: {
              name: 'array',
              generic: { name: 'array', generic: { name: 'number', nullable: false }, nullable: false },
              nullable: false,
            },
            modifiers: [],
          },
    ...
    it('maps map types with generic value type', () => {
      const ir = makeMinimalIR({
        entities: [bareEntity('MapTest', [
          { name: 'meta', type: { name: 'map', generic: { name: 'string', nullable: false }, nullable: false }, modifiers: [] },
        ])],
      });
    ...
    it('warns on unknown type and falls back to z.unknown()', () => {
      const ir = makeMinimalIR({
        entities: [bareEntity('UnknownTest', [
          { name: 'weird', type: { name: 'customType', nullable: false }, modifiers: [] },
        ])],
      });
  ...
  describe('property modifiers', () => {
    ...
    it('marks non-required properties with .optional()', () => {
      const ir = makeMinimalIR({
        entities: [bareEntity('Opt', [
          { name: 'bio', type: { name: 'string', nullable: false }, modifiers: [] },
        ])],
      });
    ...
    it('handles nullable types with .nullable()', () => {
      const ir = makeMinimalIR({
        entities: [bareEntity('Null', [
          { name: 'name', type: { name: 'string', nullable: true }, modifiers: [] },
        ])],
      });
    ...
    it('handles default values with .default()', () => {
      const ir = makeMinimalIR({
        entities: [bareEntity('Defaults', [
          { name: 'status', type: { name: 'string', nullable: false }, defaultValue: { kind: 'string', value: 'draft' }, modifiers: [] },
          { name: 'count', type: { name: 'int', nullable: false }, defaultValue: { kind: 'number', value: 0 }, modifiers: [] },
          { name: 'active', type: { name: 'boolean', nullable: false }, defaultValue: { kind: 'boolean', value: true }, modifiers: [] },
        ])],
      });
  ...
  describe('constraint refinements', () => {
    it('applies numeric range constraints as .min()/.max()', async () => {
      ...
      const zodResult = projection.generate(result.ir!, { surface: 'zod.entity', options: { emitTypes: false } });
  ...
  describe('computed properties', () => {
    it('generates computed schema extension when entity has computed props', async () => {
      ...
      const zodResult = projection.generate(result.ir!, { surface: 'zod.entity' });
      const code = firstCode(zodResult);
  ...
  describe('zod.command surface', () => {
      ...
      const ir = makeMinimalIR({
        ...
        commands: [{
          ...
          parameters: [
            ...
            { name: 'priority', type: { name: 'int', nullable: false }, required: false },
          ],
          guards: [],
          actions: [],
          emits: [],
    ...
    it('filters to specific command when request.command is set', () => {
      ...
      const result = projection.generate(ir, { surface: 'zod.command', command: 'create' });
      expect(result.artifacts).toHaveLength(1);
    ...
    it('returns error for missing command', () => {
      const ir = makeMinimalIR();
      const result = projection.generate(ir, { surface: 'zod.command', command: 'nope' });
      expect(result.artifacts).toHaveLength(0);
    ...
    it('handles command parameter defaults', () => {
      const ir = makeMinimalIR({
        commands: [{
          name: 'configure',
          parameters: [
            { name: 'timeout', type: { name: 'int', nullable: false }, required: false, defaultValue: { kind: 'number', value: 30 } },
          ],
          guards: [],
          actions: [],
          emits: [],
  ...
  describe('determinism', () => {
    it('produces identical output for identical IR (sans timestamp)', () => {
      const ir = makeMinimalIR({
        entities: [bareEntity('Recipe', [
          { name: 'id', type: { name: 'string', nullable: false }, modifiers: ['required'] },
          { name: 'title', type: { name: 'string', nullable: false }, modifiers: [] },
        ])],
      });

      const result1 = projection.generate(ir, { surface: 'zod.entity', options: { emitHeader: false } });
      const result2 = projection.generate(ir, { surface: 'zod.entity', options: { emitHeader: false } });
