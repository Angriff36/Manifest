...
function generateEntitySchema(
  ...
  if (opts.emitComputedSchemas && entity.computedProperties.length > 0) {
    ...
    for (const cp of entity.computedProperties) {
      const cpLine = generateComputedPropertyLine(
        cp,
        diagnostics,
        valueObjectMap
      );
  ...
  if (opts.emitTypes) {
    ...
    if (opts.emitComputedSchemas && entity.computedProperties.length > 0) {
      lines.push(
        `export type ${name}WithComputed = z.infer<typeof ${name}ComputedSchema>;`
      );
    }
  }
...
function generateCommandSchema(
  ...
  lines.push(
    `// Command: ${command.name}${command.entity ? ` on ${command.entity}` : ""}`
  );
  ...
  if (opts.emitTypes) {
    lines.push("");
    lines.push(
      `export type ${qualified}Params = z.infer<typeof ${schemaName}>;`
    );
  }

  // Return type schema if command has a return type
  if (command.returns && opts.emitTypes) {
    const returnExpr = irTypeToZod(
      command.returns,
      diagnostics,
      valueObjectMap
    );
    ...
    lines.push(
      `export type ${qualified}Return = z.infer<typeof ${qualified}ReturnSchema>;`
    );
  }
...
export class ZodProjection implements ProjectionTarget {
  ...
  readonly description =
    "Zod validation schemas for IR entities and command parameters";
  readonly surfaces = ["zod.entity", "zod.command", "zod.schemas"] as const;

  generate(ir: IR, request: ProjectionRequest): ProjectionResult {
    const diagnostics: ProjectionDiagnostic[] = [];
    const opts = normalizeOptions(
      request.options as ZodProjectionOptions | undefined
    );
  ...
  private generateEntitySurface(
    ...
    for (const entity of entities) {
      const result = generateEntitySchema(
        entity,
        opts,
        diagnostics,
        valueObjectMap
