 ...
 * - `clerk`: reads `@clerk/express` getAuth and attaches `req.user`
 * - `none`: sets anonymous user and continues
 *
 * Express emits a `RequestHandler`; Fastify emits a `preHandler` hook.
 */
function generateExpressAuthStub(options: NormalizedOptions): string {
  ...
  if (options.authProvider === 'none') {
    ...
    if (options.framework === 'fastify') {
      ...
      lines.push('');
      lines.push(
        `export async function ${name}(request: FastifyRequest, _reply: FastifyReply): Promise<void> {`,
      );
      lines.push("  (request as { user?: { id: string } }).user = { id: 'anonymous' };");
      lines.push('}');
    } else {
      lines.push("import type { RequestHandler } from 'express';");
      lines.push('');
      lines.push(`export const ${name}: RequestHandler = (req, _res, next) => {`);
      lines.push("  (req as { user?: { id: string } }).user = { id: 'anonymous' };");
      lines.push('  next();');
      lines.push('};');
    }
    lines.push('');
  ...
  if (options.authProvider === 'clerk') {
    ...
    lines.push('// DO NOT EDIT — generated by the Express projection (companions surface).');
    lines.push('//');
    lines.push('// Requires Clerk Express middleware (`clerkMiddleware`) mounted upstream');
    lines.push('// so `getAuth(req)` can resolve the session.');
    lines.push('');
    if (options.framework === 'fastify') {
      lines.push("import type { FastifyRequest, FastifyReply } from 'fastify';");
      lines.push("import { getAuth } from '@clerk/express';");
      lines.push('');
      lines.push(
        `export async function ${name}(request: FastifyRequest, reply: FastifyReply): Promise<void> {`,
      );
      lines.push('  const auth = getAuth(request as never);');
      ...
      lines.push('    return;');
      lines.push('  }');
      lines.push('  (request as { user?: { id: string } }).user = { id: auth.userId };');
      lines.push('}');
    } else {
      lines.push("import type { RequestHandler } from 'express';");
      lines.push("import { getAuth } from '@clerk/express';");
      lines.push('');
      lines.push(`export const ${name}: RequestHandler = (req, res, next) => {`);
      lines.push('  const auth = getAuth(req);');
      lines.push('  if (!auth.userId) {');
      ...
      lines.push('    return;');
      lines.push('  }');
      lines.push('  (req as { user?: { id: string } }).user = { id: auth.userId };');
      lines.push('  next();');
      lines.push('};');
    }
    lines.push('');
  ...
  lines.push('// Or set authProvider: "clerk" | "none" on the Express projection options.');
  lines.push('');
