...
export function pydanticEntityPathHint(entity: {
  name: string;
  module?: string;
}): string {
...
export function pydanticCommandPathHint(command: {
  name: string;
  module?: string;
}): string {
  const mod = moduleDirSegment(command.module);
  return mod
    ? `models/${mod}/commands/${command.name}.py`
    : `models/commands/${command.name}.py`;
}
