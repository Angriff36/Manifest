...
describe('pydantic pathHints — per-module nesting', () => {
  it('keeps flat models/ paths when module is absent', () => {
    ...
    expect(pydanticCommandPathHint({ name: 'CreateOrder' })).toBe('models/commands/CreateOrder.py');
  });
  ...
  it('nests command under models/<module>/commands/ when module is set', () => {
    expect(pydanticCommandPathHint({ name: 'CreateOrder', module: 'billing' })).toBe(
      'models/billing/commands/CreateOrder.py',
    );
  });

  it('sanitizes unsafe module segments', () => {
    expect(pydanticEntityPathHint({ name: 'Order', module: 'Billing / Ops!' })).toBe(
      'models/Billing_Ops/Order.py',
    );
  });

  it('treats blank module as flat', () => {
    expect(pydanticEntityPathHint({ name: 'Order', module: '   ' })).toBe('models/Order.py');
  });
});
