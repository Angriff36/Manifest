...
export const DISMISS_LABEL_RE = /\b(dismiss|close|cancel|clear|ok|okay|got it|hide|x)\b/i;
...
export function entityTokens(entity: string): string[] {
  ...
  return [
    ...new Set([entity.toLowerCase(), camel.toLowerCase(), slug, last].filter(Boolean)),
  ] as string[];
}
...
export function findIdentityAtControlSite(
  ...
  const found = findEntityIdentityIn(cap, controlSource) ?? findEntityIdentityIn(cap, window);
...
export function isWrongEntityIdentity(
  ...
  while ((m = receiverRe.exec(local)) !== null) {
    ...
    if (
      recv === 'self' ||
      recv === 'params' ||
      tokens.some((t) => recv === t || t.endsWith(recv))
    ) {
      sawEntity = true;
    } else {
      sawForeign = true;
...
export function handlerNameMatchesCommand(cap: WiringCommandDescriptor, handler: string): boolean {
  const cmd = cap.command;
  const patterns = [
    new RegExp(`\\bhandle${escape(cmd)}\\b`, 'i'),
    new RegExp(`\\bon${escape(cmd)}\\b`, 'i'),
    new RegExp(`\\b${escape(cmd)}\\b`, 'i'),
    new RegExp(`\\bset${escape(cmd[0]!.toUpperCase() + cmd.slice(1))}\\w*\\s*\\(`, 'i'),
  ];
...
export function wiredControlLabel(content: string, bindingCallee: string): string | undefined {
  const re = new RegExp(
